...
def positional_encoding(x, size, pe='normal', use_pos=False):
    comp_x = torch.sin(0. * x)
    comp_y = torch.cos(0. * x)
    # print("x", x.shape)

    for i in range(1, size):
        comp_x = torch.cat([comp_x, torch.sin((2 ** i) * math.pi * x)], dim=-1)
        comp_y = torch.cat([comp_y, torch.cos((2 ** i) * math.pi * x)], dim=-1)
        
    comp = torch.cat([comp_x, comp_y], dim=-1)
    #print(comp.shape)
    # print("p_transformed", p_transformed.shape)
...
def nerf_sphere(steps, sptype):
    if sptype == 'nerf':
        ...
        start_p = 0.
        end_p = 1.
    return torch.linspace(start_p, end_p, steps=steps)
