...
from dnnlib.geometry import (
    positional_encoding, nerf_sphere, upsample, downsample
)
...
class NeRFBlock(nn.Module):
    ...
    def transform_points(self, p, views=False):
        ...
        if self.positional_encoding == 'gauss':
            ...
            comp_x = torch.sin(0. * p)
            comp_y = torch.cos(0. * p)

            for i in range(1, L):
                comp_x = torch.cat([comp_x, torch.sin((2 ** i) * math.pi * p)], dim=-1)
                comp_y = torch.cat([comp_y, torch.cos((2 ** i) * math.pi * p)], dim=-1)
                
            p_transformed = torch.cat([comp_x, comp_y], dim=-1)
...
class VolumeRenderer(object):
    ...
    def forward_hierarchical_sampling(self, di, weights, n_steps, det=False):
        ...
        #print("F",di_fine.shape)
        return di_fine

    def backward_hierarchical_sampling(self, di, weights, n_steps, det=False):
        # print(di.shape)
        di_mid = 0.5 * (di[..., :-1] + di[..., 1:])
        # print("M", di_mid.shape)
        n_bins = di_mid.size(-1)
    ...
    def forward_rendering(self, H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles):
        ...
        # sample points
        di = nerf_sphere(steps=H.n_steps, sptype='nerf').to(H.device)
        di = repeat(di, 's -> b n s', b=H.batch_size, n=H.n_points)
        if (H.training and (not H.get('disable_noise', False))) or H.get('force_noise', False):
            di = self.C.add_noise_to_interval(di)
        di_trs = self.C.get_transformed_depth(di)
    ...
    def forward_rendering_background(self, H, output, bg_nerf, nerf_input_cams, latent_codes, styles_bg):
        ...
        di = nerf_sphere(steps=H.n_bg_steps, sptype='nerfplusplus').to(H.device)
        ...
        if self.hierarchical and (not H.get('disable_hierarchical', False)):
            with torch.no_grad():
                di_fine = self.backward_hierarchical_sampling(di, bg_weights, H.n_bg_steps, det=(not H.training))
            di_trs = self.C.get_transformed_depth(di)
            di_trs_fine = self.C.get_transformed_depth(di_fine)
            p_bg, r_bg = self.C.get_evaluation_points_bg(pixels_world, camera_world, di_trs_fine)
            # if nerf_input_feats is not None:
            #     p_f = self.I.query_input_features(p_f, nerf_input_feats, bg_shape, bound)

            feat_f, sigma_raw_f = bg_nerf(p_bg, r_bg, z_shape_bg, z_app_bg, ws=styles_bg, shape=bg_shape)
            feat_f      = rearrange(feat_f, 'b (n s) d -> b n s d', s=H.n_bg_steps)
            sigma_raw_f = rearrange(sigma_raw_f.squeeze(-1), 'b (n s) -> b n s', s=H.n_bg_steps)
            sigma_f     = self.get_density(sigma_raw_f, bg_nerf, training=H.training)
            ...
            bg_weights, bg_lambda = self.C.calc_volume_weights(
                sigma, di if self.C.dists_normalized else di_trs,  # use real dists for computing weights, 
                ray_vector, last_dist=0 if not H.fg_inf_depth else 1e10)[:2]
